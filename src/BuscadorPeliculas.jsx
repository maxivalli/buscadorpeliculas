      ...
      <div className="movie-list">
        {peliculas.map((pelicula) => (
          <div key={pelicula.id} className="movie-card">
            <img src={pelicula.poster_path ? `https://image.tmdb.org/t/p/w500${pelicula.poster_path}` : imagenGenerica} alt={pelicula.title}/>
            <h2>{pelicula.title}</h2>
            <h3>Estreno: {pelicula.release_date}</h3>
            <Link className="buttonSinopsis" to={`/detail/${pelicula.id}`}>
              ver sinopsis
            </Link>
          </div>
        ))}
